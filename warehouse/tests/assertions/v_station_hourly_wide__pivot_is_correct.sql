-- Assert: Pivot is correct for known station-hour combinations
-- Check station_a at 10:00 has all three pollutants correctly pivoted
...
SELECT
  'v_station_hourly_wide__pivot_is_correct' AS test_name,
  ...
  'Pivoted values do not match expected' AS reason
FROM expected e
LEFT JOIN actual a ON e.station_id = a.station_id AND e.hour_utc = a.hour_utc
WHERE a.pm25_value IS NULL OR a.pm25_value != e.expected_pm25
