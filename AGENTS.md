...
tests/
  ...
  bq_views/         # BQ view test runner (creates temp datasets, seeds fixtures, runs SQL assertions)
warehouse/
  raw/              # DDL for raw BigQuery tables
  staging/          # Dedup views (v_openaq_deduped, v_weather_deduped)
...
```bash
cd tests/bq_views && python run_view_tests.py
```
